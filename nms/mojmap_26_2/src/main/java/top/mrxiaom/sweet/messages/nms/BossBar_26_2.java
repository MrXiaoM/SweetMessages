...
public class BossBar_26_2 implements IBossBar {
    ...
    public void setTitle(BossBar bossBar, Component component) {
        ...
        ComponentSerialization.CODEC.parse(JsonOps.INSTANCE, json)
                .result()
                .ifPresent(nms::setName);
    }

    public Component getTitle(BossBar bossBar) {
        ServerBossEvent nms = ((CraftBossBar) bossBar).getHandle();
        if (nms == null) return null;
        return ComponentSerialization.CODEC.encodeStart(JsonOps.INSTANCE, nms.getName())
                .result()
                .map(serializer::deserializeFromTree)
                .orElse(null);
    }
}
